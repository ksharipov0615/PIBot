...
async def send_recap_email(r: dict, subject_prefix: str) -> None:
    ...
    callers of this function — are SHARED between the WSF (User C) and MOP
    (Admin-as-pricer) flows, so this function fires for both workflows. User
    C must only ever be CC'd on WSF requests, never MOP. Silently skips if
    ...
    if workflow != "MOP" and USER_C_EMAIL:
