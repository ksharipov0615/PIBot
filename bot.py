"""
Price Indication Bot — v3
Workflow:
  Sales Representative → User C (availability check)
    └─ Not available  → Representative (done)
    └─ Available      → User L (logistics) → User C (pricing) → Representative

Changes over v2:
  - New workflow: request goes to User C first for availability validation;
    only if available does it proceed to User L, then back to User C for pricing.
  - New DB status 'pending_C_avail' for the initial availability-check stage.
  - New DB column c_avail_at (timestamp when C confirmed availability).
  - Admin can now also act as a Sales Representative (/newrequest allowed).
  - Freight label changed from per-container to per-mt (Freight/mt).
  - Reminder job extended to cover pending_C_avail stage.
  - admin_stage_copy gains a 'c_avail' stage for the availability confirmation.
"""
...
def admin_stage_copy(stage: str, r: dict) -> str:
    header = {
        ...
        "c_avail": "🔔 <b>[ADMIN] User C confirmed availability — forwarded to User L</b>",
...
async def pending(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with sqlite3.connect(DB_PATH) as con:
        ...
        if uid == USER_L_ID:
            ...
            rows = con.execute(
                "SELECT * FROM requests WHERE status IN ('pending_C_avail', 'pending_C') ORDER BY id"
            ).fetchall()
            if not rows:
                await update.message.reply_text("✅ No pending requests for you.")
                return
...
async def c_availability(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    msg = await ctx.bot.send_message(
        USER_L_ID,
        f"📥 <b>Request #{req_id} — Availability confirmed by User C</b>\n\n"
        ...
        for row in rows_c_avail:
            ...
            try:
                await ctx.bot.send_message(
                    USER_C_ID,
                    f"⏰ <b>Reminder</b> — Request #{r['id']} is awaiting your <b>availability check</b> for 24 h!\n\n"
                    f"Use /pending to respond.\n\n" + rep_recap(r),
                    parse_mode="HTML",
                )
                con.execute(
                ...
                logger.error(f"Reminder C_avail error for request #{r['id']}: {exc}")
