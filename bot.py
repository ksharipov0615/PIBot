...
def _fetch_report_rows(range_key: str) -> list[dict]:
    cutoff = _report_cutoff(range_key)
    with sqlite3.connect(DB_PATH) as con:
        con.row_factory = sqlite3.Row
        if cutoff is None:
            rows = con.execute("SELECT * FROM requests ORDER BY id").fetchall()
        else:
            rows = con.execute(
                "SELECT * FROM requests WHERE created_at >= ? ORDER BY id", (cutoff,)
            ).fetchall()
    return [dict(row) for row in rows]


def _build_report_summary_html(rows: list[dict], range_label: str) -> str:
    total = len(rows)
    ...
    return (
        f"📊 <b>Pricing Bot Report — {e(range_label)}</b>\n\n"
...
async def report_range_selected(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(f"✅ Report sent to {ADMIN_EMAIL} — {range_label}.")
