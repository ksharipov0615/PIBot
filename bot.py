...
# Price-negotiation flow (50–53): rep's counter-price entry, Admin's reprice
# entry (new price, then new validity)
REP_COUNTER_PRICE, ADMIN_REPRICE, ADMIN_REPRICE_VALIDITY = range(50, 53)
...
async def counter_decline(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    ...
    update_request(req_id, rep_response="declined_by_admin", rep_responded_at=now_utc())
    r = get_request(req_id)
    ...
    # Deactivate: Admin's own Reprice/Turn down buttons are removed too.
    await query.edit_message_text(
...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(counter_decline, pattern=r"^counter_decline_\d+$"))
