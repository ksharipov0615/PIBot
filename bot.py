...
async def send_recap_email(r: dict, subject_prefix: str) -> None:
    """Send the price-recap email to the requesting rep, CC'ing Admin, for one
    of the three final-completion points (full price, no availability, no
    logistics service). Silently skips if email is disabled or the rep has no
    mapped address — this must never interrupt the Telegram flow."""
    if not EMAIL_ENABLED:
        return
    ...
    if not rep_email:
        ...
        return
    subject = f"{subject_prefix} — Request #{r['id']}"
    html_body = _email_html(subject, c_answer_for_rep(r))
    cc = [ADMIN_EMAIL] if ADMIN_EMAIL else None
