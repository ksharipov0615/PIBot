...
#   GMAIL_APP_PASSWORD=xxxxxxxxxxxxxxxx   (16-char App Password, NOT your normal password)
#   REP_EMAILS=111111111:alice@example.com,222222222:bob@example.com
#   ADMIN_EMAIL=admin@example.com
# Reps without a mapped email simply won't receive a recap email (bot keeps working).
# Admin is CC'd on every recap email that goes out; if ADMIN_EMAIL is unset, no CC is added.
GMAIL_ADDRESS = os.getenv("GMAIL_ADDRESS", "")
GMAIL_APP_PASSWORD = os.getenv("GMAIL_APP_PASSWORD", "")
ADMIN_EMAIL = os.getenv("ADMIN_EMAIL", "")
EMAIL_ENABLED = bool(GMAIL_ADDRESS and GMAIL_APP_PASSWORD)
...
for _pair in _rep_emails_raw.split(","):
    ...
    if not _pair or ":" not in _pair:
        continue
    _id_str, _email = _pair.split(":", 1)
    _id_str, _email = _id_str.strip(), _email.strip()
    if _id_str.isdigit() and _email:
        REP_EMAILS[int(_id_str)] = _email

if not EMAIL_ENABLED:
    logging.getLogger(__name__).warning(
        "GMAIL_ADDRESS / GMAIL_APP_PASSWORD not set — recap emails are disabled."
    )
if EMAIL_ENABLED and not ADMIN_EMAIL:
    logging.getLogger(__name__).warning(
        "ADMIN_EMAIL not set — recap emails will NOT be copied to Admin."
    )
...
def _html_to_plain(text: str) -> str:
    """Strip Telegram-style HTML tags for a plain-text email fallback."""
    plain = re.sub(r"<[^>]+>", "", text)
    return html.unescape(plain)


def _send_via_starttls(msg, to_addrs: list[str], cc_addrs: list[str], timeout: float) -> None:
    with smtplib.SMTP("smtp.gmail.com", 587, timeout=timeout) as server:
        server.ehlo()
        server.starttls(context=ssl.create_default_context())
        server.ehlo()
        server.login(GMAIL_ADDRESS, GMAIL_APP_PASSWORD)
        server.sendmail(GMAIL_ADDRESS, to_addrs + cc_addrs, msg.as_string())


def _send_via_ssl(msg, to_addrs: list[str], cc_addrs: list[str], timeout: float) -> None:
    with smtplib.SMTP_SSL("smtp.gmail.com", 465, timeout=timeout, context=ssl.create_default_context()) as server:
        server.ehlo()
        server.login(GMAIL_ADDRESS, GMAIL_APP_PASSWORD)
        server.sendmail(GMAIL_ADDRESS, to_addrs + cc_addrs, msg.as_string())


def _send_email_sync(to_addrs: list[str], cc_addrs: list[str], subject: str, html_body: str,
                      timeout: float = 15.0) -> None:
    """Blocking SMTP send — must be run off the event loop (see send_recap_email).

    Tries STARTTLS on port 587 first, then falls back to implicit SSL on port 465.
    Some hosts/containers block one outbound SMTP port but not the other, so trying
    both maximizes the chance of getting through. On failure, raises a RuntimeError
    with a message that distinguishes "can't reach Gmail at all" (network/firewall)
    from "reached Gmail but auth/send failed" (credentials/app password).
    """
    ...
    try:
        _send_via_starttls(msg, to_addrs, cc_addrs, timeout)
        return
    except (OSError, socket.timeout) as e:
        # Network-level failure (unreachable, refused, timed out, DNS, etc.) — try the fallback port.
        network_errors.append(f"port 587 (STARTTLS): {e!r}")
    except smtplib.SMTPException:
        # We reached Gmail but auth/protocol failed — no point retrying on another port.
        raise

    try:
        _send_via_ssl(msg, to_addrs, cc_addrs, timeout)
        return
    except (OSError, socket.timeout) as e:
        network_errors.append(f"port 465 (SSL): {e!r}")
        raise RuntimeError(
            "Could not reach smtp.gmail.com on either port 587 or 465 — this looks like an "
            "outbound network/firewall issue on the host running the bot, not a code or "
            "credentials problem. Details: " + "; ".join(network_errors)
        ) from e


async def send_recap_email(r: dict, body_html: str) -> None:
    """Email the final price recap to the requesting rep, always CC'd to Admin.

    - If the rep has a mapped email: rep is To, Admin (if configured) is Cc.
    - If the rep has no mapped email but Admin is configured: Admin still gets it (To),
      so Admin never misses a recap even when a rep's address isn't on file.
    - Never raises — a failed/missing email must not break the Telegram flow.
    """
    if not EMAIL_ENABLED:
        return

    rep_addr = REP_EMAILS.get(r.get("rep_id"))
    admin_addr = ADMIN_EMAIL or None

    if rep_addr and admin_addr and admin_addr != rep_addr:
        to_addrs, cc_addrs = [rep_addr], [admin_addr]
    elif rep_addr:
        to_addrs, cc_addrs = [rep_addr], []
    elif admin_addr:
        logger.warning(
            "No email mapped for rep_id=%s (Request #%s) — sending recap to Admin only.",
            r.get("rep_id"), r.get("id"),
        )
        to_addrs, cc_addrs = [admin_addr], []
    else:
        logger.warning(
            "No email mapped for rep_id=%s and no ADMIN_EMAIL set (Request #%s) — skipping recap email.",
            r.get("rep_id"), r.get("id"),
        )
        return

    subject = f"Price Reply — Request #{r['id']} [{r.get('workflow') or 'WSF'}] {r.get('product', '')}"
    try:
        await asyncio.to_thread(_send_email_sync, to_addrs, cc_addrs, subject, body_html)
        logger.info(
            "Recap email sent for Request #%s to %s (cc: %s)",
            r["id"], to_addrs, cc_addrs or "—",
        )
    except Exception:
        logger.exception("Failed to send recap email for Request #%s to %s", r["id"], to_addrs)
...
async def c_confirm_no_service(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.text == "Yes, send reply":
        ...
        await send_recap_email(r, c_answer_for_rep(r))
        await update.message.reply_text(
