...
WSF-only additions:
  - After User C confirms availability (Yes), User C now enters STP tariff/mt and
    Novo tariff/mt (both numeric, both may be 0) before the request is forwarded to
    User L. New DB columns 'stp_tariff' / 'novo_tariff' (TEXT), non-destructive migration.
    MOP/NP is unaffected — Admin's availability confirmation still forwards immediately.
  - User L's POL step is now a selector (STP / Novo buttons) instead of free text.
    User L's choice determines which tariff/mt is added to Subtotal/mt to produce
    Total/mt. MOP/NP's POL step remains free text, unaffected.
"""
...
# User C flow (30–38)
(
    C_AVAILABILITY, C_STP_TARIFF, C_NOVO_TARIFF,
    C_VOLUME, C_PRICE, C_ETD, C_VALIDITY, C_COMMENTS,
) = range(30, 38)
C_CONFIRM_NO_SERVICE = 38
...
def _wsf_grand_total_per_mt(r: dict, subtotal: str) -> str:
    ...
    applies, based on User L's POL selection (STP or Novo). Returns '—' if
    the subtotal or the selected tariff isn't available/numeric."""
    if subtotal == "—":
        return "—"
    pol = r.get("pol")
    ...
    try:
        ...
        grand_total = float(subtotal) + tariff_val
...
async def send_email_via_gmail(
    ...
    if not EMAIL_ENABLED:
        return
    try:
        await asyncio.to_thread(_send_email_sync, to_addr, subject, html_body, cc)
    except Exception:
...
async def send_recap_email(r: dict, subject_prefix: str) -> None:
    ...
    workflow = r.get("workflow", "WSF")
    cc_addrs = [ADMIN_EMAIL] if ADMIN_EMAIL else []
    if workflow != "MOP/NP" and USER_C_EMAIL:
...
async def r_packaging(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if workflow == "WSF":
        ...
        # MOP/NP: pallets only relevant for 1000 kg packaging
...
async def _confirm_availability_and_forward_to_l(
    ...
    STP/Novo tariff steps) — extra_fields carries stp_tariff/novo_tariff
    for WSF, and is empty for MOP/NP."""
