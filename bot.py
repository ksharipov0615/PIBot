...
async def send_recap_email(r: dict, subject_prefix: str) -> None:
    """Send the price-recap email to the requesting rep, CC'ing Admin and
    User C, for one of the three final-completion points (full price, no
    availability, no logistics service). This path is WSF-only — User C's
    interface never surfaces MOP requests, so every caller of this function
    is already on the WSF flow. Silently skips if email is disabled or the
    rep has no mapped address — this must never interrupt the Telegram
    flow."""
    if not EMAIL_ENABLED:
        return
    ...
    cc = [addr for addr in (ADMIN_EMAIL, USER_C_EMAIL) if addr] or None
