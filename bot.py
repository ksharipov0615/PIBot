...
# Hard cap on how long a single email send may take before it's treated as
# failed. Prevents a slow/unresponsive Gmail connection from ever stalling
...
async def _send_email_async(to_addrs: list[str], cc_addrs: list[str], subject: str, html_body: str) -> None:
    """Fully async SMTP send over port 587/STARTTLS — never blocks the event loop.

    Uses aiosmtplib (native asyncio SMTP client) instead of wrapping the
    blocking stdlib smtplib in a thread. A hard timeout guarantees this
    coroutine always returns within EMAIL_TIMEOUT_SECONDS, even if Gmail's
    servers hang or the connection stalls — so the bot can never freeze or
    time out waiting on an email.
    """
    ...
    async def _do_send() -> None:
        smtp = aiosmtplib.SMTP(hostname="smtp.gmail.com", port=587, start_tls=False)
        await smtp.connect()
        try:
            await smtp.ehlo()
            await smtp.starttls()
            ...
            await smtp.sendmail(GMAIL_ADDRESS, to_addrs + cc_addrs, msg.as_string())
        finally:
            await smtp.quit()

    await asyncio.wait_for(_do_send(), timeout=EMAIL_TIMEOUT_SECONDS)
...
async def send_recap_email(r: dict, body_html: str) -> None:
    ...
    try:
        await _send_email_async(to_addrs, cc_addrs, subject, body_html)
        logger.info(
        ...
        logger.error(
            "Recap email timed out after %ss for Request #%s to %s — Gmail did not respond in time.",
            EMAIL_TIMEOUT_SECONDS, r["id"], to_addrs,
        )
    except Exception:
