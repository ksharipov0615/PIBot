...
# Price-negotiation flow (50–54): rep's counter-price entry, Admin's reprice
# entry (new price, then new validity), Admin's decline-with-new-validity
REP_COUNTER_PRICE, ADMIN_REPRICE, ADMIN_REPRICE_VALIDITY, ADMIN_DECLINE_VALIDITY = range(50, 54)
...
async def price_counter_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Remembered so price_counter_received can deactivate THIS message's buttons
...
async def price_counter_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    update_request(req_id, rep_response="countered", counter_price=text, rep_responded_at=now_utc(), active_price_msg_id=None)
    r = get_request(req_id)
    ...
    await update.message.reply_text(
        f"💬 Counter of <code>{e(text)} USD/mt</code> sent for Request #{req_id}. "
    ...
    await ctx.bot.send_message(
        ADMIN_ID,
        f"💬 <b>Request #{req_id} — Rep countered</b>\n\n"
        f"Original price/mt: <code>{e(r['price'])} USD</code>\n"
        f"Counter price/mt: <code>{e(text)} USD</code>\n\n"
        f"Product: <code>{e(r['product'])}</code>  |  Volume: <code>{e(r['volume'])} mt</code>  |  POD: <code>{e(r['pod'])}</code>",
        parse_mode="HTML",
    ...
    if r.get("workflow") == "WSF":
        try:
            await ctx.bot.send_message(
                ...
                f"Counter price/mt: <code>{e(text)} USD</code>\n\n"
                f"Product: <code>{e(r['product'])}</code>  |  Volume: <code>{e(r['volume'])} mt</code>  |  POD: <code>{e(r['pod'])}</code>\n\n"
                f"<i>Admin will decide whether to reprice or turn this down.</i>",
                parse_mode="HTML",
            )
        except Exception:
...
async def counter_decline_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Deactivate: Admin's own Reprice/Turn down buttons are removed as soon as
    # Turn down is chosen, so they can't be double-tapped mid-entry.
    try:
        await query.edit_message_reply_markup(reply_markup=None)
    except Exception:
        logger.exception("Failed to clear buttons on Admin counter-notification for Request #%s", req_id)

    ctx.user_data["decline_req"] = req_id
    await query.message.reply_text(
        f"Request #{req_id} — turning down the counter, original price "
...
async def counter_decline_validity_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    update_request(
        req_id, validity=text, rep_response="declined_by_admin", rep_responded_at=now_utc(),
    )
    r = get_request(req_id)

    # Deactivate: turning a counter down closes the negotiation — the rep's
    # message carries no buttons at all (contrast with Reprice, which gets a
    # fresh set).
    await ctx.bot.send_message(
        r["rep_id"],
    ...
    await update.message.reply_text(
        f"🚫 Counter turned down for Request #{req_id}. Original price stands, "
...
async def counter_reprice_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Deactivate: Admin's own Reprice/Turn down buttons are removed as soon as
    # Reprice is chosen, so they can't be double-tapped mid-entry.
    try:
        await query.edit_message_reply_markup(reply_markup=None)
    except Exception:
        logger.exception("Failed to clear buttons on Admin counter-notification for Request #%s", req_id)
...
async def counter_reprice_validity_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    update_request(
        req_id, price=price, validity=text, rep_response=None, counter_price=None,
        c_answered_at=now_utc(), validity_reminder_sent=None,
    )
    r = get_request(req_id)

    await update.message.reply_text(f"✅ Revised price sent to representative for Request #{req_id}.")
    # Fresh set of buttons — this reopens the negotiation for another round.
    msg = await ctx.bot.send_message(
        r["rep_id"],
        f"🔁 <b>Request #{req_id} — Revised price</b>\n\n" + c_answer_for_rep(r),
        parse_mode="HTML",
...
def main() -> None:
    ...
    counter_conv = ConversationHandler(
        ...
        states={REP_COUNTER_PRICE: [MessageHandler(TEXT_FILTER, price_counter_received)]},
        fallbacks=[CommandHandler("cancel", cancel)],
        allow_reentry=True,
        per_user=True,
    )
